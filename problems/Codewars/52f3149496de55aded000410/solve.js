//	Code available at https://codewars.com/kata/reviews/52f3149596de55aded000413/groups/624cce5301bc63000116952f in accordance with https://docs.codewars.com/community/rules/#rules

`!@#$%^& ()_+[];'./,\<>?:" }
  aBcDeF HiJkLmNoPqRsTuVwXyZ0123456789`!@#$%^&*()_+[];'./,\ >? "{ |=- b dEf hI
K



#$%^ *( _
	];'./ \ >?:"{} = a cDeFgHiJkLmNoPq
	
	VwXyZ0123456789 !@#$%^& () +[ ;
	  \<>?:"{}|=- bCd fGhIj lMnOpQr tU Wx z
	    6789`!@#$%^&*()_+[];'./,\<>?:"{}| -aB
	    iJkLmNoPqRsTuVwXyZ0123456789`!@#$ ^&*(
	    './,\<>?:"{}|=-AbCdEfGhIjKlMnOpQrS UvW
	    3456789`!@#$%^&*()_+[];'./,\<>?:"{}|=-aBc eFgH
	    oPqRsTuVwXyZ0123456789`!@#$%^&*( _+[
	    \<>?:"{}|=-AbCdEfGhIjKlMnOpQrStUvW Yz01
	    89`!@#$%^&*()_+[];'./,\<>?:"{}|=-aBcDeFg iJk
	    RsTuVwXyZ0123456789`!@#$%^&*()_+[];'./,\ >?:"
	    bC
	  hI
	
	nOpQrStUvWxYz012 456789` @# %^ *
	             
	  |=-aBcDe gHiJkLmNoPqR TuVwX
	    456789 !@#$%^&*()_+[];'./,\<>?: { |=-A C EfG I KlM O QrS
	  x
	     
	  9`!@#$% & ( _+ ] ' /,\< ?:"{}
	    D F HiJkLmNoPqRsTuVwXyZ01 3456789`!@ $%^&* )_+[];'./,\< ?:"{}|=-Ab dEfGh
	    OpQrStUv x z0123456789`!@#$%^&*()_+[];'./ \< ?: { | -aB De
	    Lm oPqRsTuVwXyZ01234 6789`! #$%^& ()_+[];'./,\< ?: {} =
	      hIjKlMnOpQrStUvWxYz0123456789`!@ $%^&*()_+ 
	    \<
	   }
	  Bc
eFgHi